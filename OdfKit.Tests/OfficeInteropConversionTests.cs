...
#pragma warning restore CA1416
    ...
    private static void AssertPdfVisualDifferenceBelow(string expectedPdfPath, string actualPdfPath, double thresholdPercent)
    {
        ...
        string scriptPath = Path.Combine(Path.GetTempPath(), "OdfKitPdfVisualDiff_" + Guid.NewGuid().ToString("N") + ".py");
        File.WriteAllText(scriptPath, PdfVisualDiffScript);
        try
        {
            var startInfo = new ProcessStartInfo
            {
    ...
    private const string PdfVisualDiffScript = """
import sys
import numpy as np
