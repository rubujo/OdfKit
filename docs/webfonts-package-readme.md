...
- 接受 TTF／OTF／TTC／OTC 指定 face、`.tte`、WOFF 與受控 WOFF2 作為輸入，輸出瀏覽器部署用
  ...
  variable `OTTO` 為 experimental；名稱式 CFF、無 VariationStore 的 CFF2、直接 collection
  collection 輸出明確拒絕；color font 採保留完整 glyph ID 空間的 experimental 路徑。
