...
| typed DOM wrapper | ODF 1.4 schema element / attribute metadata | `OdfNodeFactory`、generated DOM wrappers、`OdfTypedDomCoverage`、`eng/Test-OdfTypedDomCoverage.ps1` | `TypedDomParityTests`、`OdfToolkitParityReadinessTests`、`docs/typed-dom-coverage.md` | partial | 已有 wrapper / factory / attribute coverage guard、schema-to-wrapper report 與 CI artifact；尚需 typed datatype 與 ODFDOM sample parity。 |
