...
| typed DOM wrapper | ODF 1.4 schema element / attribute metadata | `OdfNodeFactory`、generated DOM wrappers | `TypedDomParityTests`、`docs/typed-dom-coverage.md` | partial | 已有 wrapper / factory / attribute coverage guard；尚需完整 schema-to-wrapper report 與 typed datatype。 |
...
| 外部 ODF Validator baseline | ODF Toolkit / ODF Validator valid-invalid classification | `OdfExternalValidator`、CLI `validate --baseline`、`validate-corpus`、`--baseline-exceptions` | `CliTests`、`docs/odf-toolkit-parity.md` | validated | 已可執行 manifest、比對 kind / version 並記錄 documented exceptions；尚需加入官方與真實 corpus 對照結果。 |
