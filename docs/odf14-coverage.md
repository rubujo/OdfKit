...
| 外部 ODF Validator baseline | ODF Toolkit / ODF Validator valid-invalid classification | `OdfExternalValidator`、CLI `validate --baseline`、`validate-corpus`、`--baseline-exceptions` | `CliTests`、`docs/odf-toolkit-parity.md` | validated | 已可執行 manifest 與記錄 documented exceptions；尚需加入官方與真實 corpus 對照結果。 |
