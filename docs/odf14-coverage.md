...
| typed DOM wrapper | ODF 1.4 schema element / attribute metadata | `OdfNodeFactory`、generated DOM wrappers、`OdfTypedDomCoverage` | `TypedDomParityTests`、`docs/typed-dom-coverage.md` | partial | 已有 wrapper / factory / attribute coverage guard 與 schema-to-wrapper report；尚需 typed datatype 與 ODFDOM sample parity。 |
