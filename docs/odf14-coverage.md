...
| typed DOM wrapper | ODF 1.4 schema element / attribute metadata | `OdfNodeFactory`、generated DOM wrappers、`OdfTypedDomCoverage`、typed attribute helpers、`eng/Test-OdfTypedDomCoverage.ps1` | `TypedDomParityTests`、`OdfToolkitParityReadinessTests`、`OdfSchemaGeneratorTests`、`docs/typed-dom-coverage.md` | partial | 已有 wrapper / factory / attribute coverage guard、schema-to-wrapper report、CI artifact、常用 datatype helper 與 generator typed datatype 輸出；尚需重產生 generated wrappers、擴大 datatype 類別與 ODFDOM sample parity。 |
