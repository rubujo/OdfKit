...
| **兩個以上**尾端可選參數 | 明確多載鏈或 **options 物件** | 高頻已用 options；其餘已展開為短多載轉呼叫 + 最長無 `=`（`Expand-OptionalParameters.py --dry-run` = 0）。新 API 禁止再加「多可選位置參數」 |
...
> 本專案對「恰好一個可選參數」偏好改為**無 `=` 的明確多載鏈**；多可選優先 **options 物件**，否則完整展開多載鏈（0.x 不保留舊多可選相容層）。
...
- `python eng/Expand-OptionalParameters.py`：展開公開／保護方法上的尾端可選參數為明確多載鏈（略過 primary ctor、`out`／`ref`）；維護時以 `--dry-run` 確認為 0。  
- `python eng/Rewrite-ConvenienceSummaries.py`：高頻檔案便利多載摘要差異化（參數名進 summary）。
