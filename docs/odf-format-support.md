...
- `.odc` 已有圖表類型、標題、圖例位置、資料來源標籤、分類範圍與序列摘要 API；`.odf` 已有 MathML XML 讀寫、根節點與純文字摘要 API；`.odi` 已有主要影像路徑、媒體類型與大小摘要 API；`.odb` 已有連線參照、資料表摘要、查找與移除 API。完整圖表、MathML、影像與資料庫語意模型仍屬後續擴充。
