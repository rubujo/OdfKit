...
`OdfKit/Compliance/OdfLocalizer.Exceptions.cs` 為例外字典**入口**。
v0.0.1 起，各語系字串的**來源真相**為 `OdfKit/Compliance/i18n/exceptions.<culture>.json`；
`OdfLocalizer.Exceptions.<culture>.cs` 由
`eng/Generate-LocalizerExceptionsFromJson.ps1` 產生（`// <auto-generated />`，**請勿手改**）。
新增或修改錯誤訊息時，應編輯 JSON（或 `eng/Add-LocalizerKey.ps1`）後重產 C#，並同步全部
17 語系（`en`, `zh-TW`, `da`, `de`, `fr`, `it`, `ko`, `ms`, `nb`, `nl`, `pt`, `sk`, `ja`, `es`, `cs`, `pl`, `pt-BR`）。
不得只修改單一文化或在呼叫端硬編碼例外訊息。見 [i18n/README.md](../../OdfKit/Compliance/i18n/README.md)
...
文字，必須透過 `OdfLocalizer.GetMessage` 取得 17 語系同步資源。
