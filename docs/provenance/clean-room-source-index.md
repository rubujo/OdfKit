...
| 範圍 | 權威來源 | OdfKit 實作入口 | Golden / regression 證據 |
|---|---|---|---|
| TDF ODF Toolkit operations wire shape | TDF ODF Toolkit 公開文件與 reference JSON | `OdfKit.Extensions.Collaboration/OdtOperationsImporter.cs`、`OdtOperationsExporter.cs` | `CollaborationOperationsTests`、`tests/fixtures/collaboration/manifest.json` |
| TDF `{ "changes": [...] }` envelope 與裸陣列相容 | TDF ODF Toolkit reference operations corpus wire shape | `OdtOperationCompatibilityOptions`、`OdtOperationImportReport` | `CollaborationOperationsTests.Merge_AcceptsTdfChangesEnvelope`、`ExportToJson_TdfEnvelope_WrapsOperationsInChanges` |
| 子集合 replay：段落、文字、Tab、換行、基本格式、單段落刪除、最上層 split/merge、基本清單與固定尺寸表格 | TDF operation 名稱與公開 JSON fixture 行為觀察 | `OdtOperationsImporter.Merge` | `CollaborationOperationsTests.Merge_Replays*` |
...
Java ODF Toolkit 原始碼。完整多人協同演算法、OT、CRDT、undo stack、任意衝突合併、drawing
operation、動態表格擴張與 header/footer/note selection 完整語意仍屬非目標。
