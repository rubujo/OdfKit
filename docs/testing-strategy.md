...
真實 LibreOffice 互通測試需透過 `pwsh eng/Test-LibreOfficeInterop.ps1` 執行；該腳本會設定
`ODFKIT_RUN_LIBREOFFICE_INTEROP=1`。一般 `dotnet test` 即使本機安裝 LibreOffice，也不會自動
跑這批耗時實機測試，以避免完整回歸與 CI 快速路徑被外部程序拖慢。
...
| Trimming smoke | 驗證裁剪後主要 API 根仍可執行，並保留 BouncyCastle OpenPGP trimming guard | `tools/OdfKit.TrimSmoke`、`eng/Test-TrimSmoke.ps1`、`.github/workflows/trim-smoke.yml` |
...
- Trimming 煙霧測試：`pwsh eng/Test-TrimSmoke.ps1 -Configuration Release`。此入口是
  PublishTrimmed 實機執行檢查；`-PublishAot` 僅供研究 Native AOT 缺口，不作為本輪完成條件。
