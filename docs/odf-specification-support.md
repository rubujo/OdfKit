...
| OpenFormula | 結構性支援＋受控評估 | ODF 1.2～1.4 的 `of:=` 與 ODF 1.0／1.1 常見的 `oooc:=` 語法剖析、參照範圍／交集／聯集、引號標籤、自動交集、文件／工作表／外部名稱、inline array、矩陣公式寫回、複數、Bessel 高階數值、奇數票息、pivot 兩種語法、Small 110／110、Medium 272／272、Large 388／388 累計強制函式名稱覆蓋、求值分級、活頁簿內容模型，以及執行個體範圍自訂函式與整式後援；未知函式仍可原樣保留並產生診斷。 | 函式名稱覆蓋不等於正式 Small／Medium／Large 一致性；`DDE` 是唯一明確的 `BestEffortFunctions` 項目，核心只驗證引數形狀並依安全政策拒絕外部執行，且仍須以持續擴充的完整 corpus 驗證所有函式限制、locale 與極端值。 |
