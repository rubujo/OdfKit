...
4. **追蹤發佈進度**：
   ...
   - 產生並以官方 JSON Schema 驗證完整 release set 的 SPDX 3.0.1 JSON-LD SBOM；由於
     GitHub artifact attestation 目前仍以 SPDX 2.3 predicate 驗證，另產生只供 attestation
     使用的 2.3 相容檔。
   - 對套件、雜湊 manifest、兩種完整 SBOM、WebFont SBOM 與彙整 ZIP 建立 GitHub
     Sigstore provenance；全部 nupkg 另繫結 SPDX 2.3 相容 SBOM attestation。
   - 自動建立 GitHub Release，並利用 `GITHUB_TOKEN` 上傳 `.nupkg`、`.snupkg`、`SHA256SUMS`、
     主要 SPDX 3.0.1 SBOM、2.3 attestation 相容 SBOM、WebFont SBOM 與 ZIP 資產。
...
所有第三方 workflow 元件都以完整 40 字元 commit SHA 固定，同行版本註解保留可讀性。
Dependabot 每週更新 `github-actions`，`eng/Test-GitHubActionsPolicy.ps1 -Online` 再確認
版本註解、官方最新穩定 release（CodeQL 使用官方浮動 major tag）與 SHA 三者一致。
artifact 只保留一天；NuGet restore cache 依作業系統與明確 revision 分區，不依 CPU 架構、
RID 或任意專案檔雜湊重複建立。
...
WebFont 所有 direct package 的最新穩定版本，並委派全專案 GitHub Actions SHA 政策檢查。
Preview 只能存在於具精確版本、理由、移除條件與複查期限的例外。規範政策
