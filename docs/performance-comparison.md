...
行程執行一次。量測日期 **2026-07-09**，於導入共用 `OdfRawXmlWriter` 熱路徑
（`OdsStreamWriter`／`OdtStreamWriter` 共用）後以
`pwsh eng/Benchmark-Competitive.ps1` 連續執行兩次；**下表採用第二次**（第一次
較受冷啟動與背景負載影響，耗時明顯偏高；兩次配置量與峰值工作集差異見第 5 節）。

| 情境 | 套件（授權） | 輸出格式 | 耗時 | GC 累積配置量 | 峰值工作集 | 輸出檔案大小 |
|------|--------------|----------|------|----------------|------------|--------------|
| `OdsStreamWriter` | OdfKit（CC0-1.0） | `.ods` | 4,955 ms | **472.4 MB** | **38.1 MB** | 95.3 MB |
| `MiniExcel` | MiniExcel 1.45.0（Apache-2.0） | `.xlsx` | **4,920 ms** | 3,358.3 MB | 47.6 MB | 111.0 MB |
| `ClosedXml` | ClosedXML 0.105.0（MIT，DOM 對照組） | `.xlsx` | 38,837 ms | 10,953.4 MB | 2,180.8 MB | 65.2 MB |

（粗體標示各欄位表現最佳者。耗時欄 MiniExcel 僅快約 0.7%，可視為本機誤差內接近持平。）

兩次手動量測對照（便於評估穩定性；單位同表）：

| 情境 | 第 1 次耗時 | 第 2 次耗時 | 第 1 次配置 | 第 2 次配置 | 第 1 次峰值 | 第 2 次峰值 |
|------|-------------|-------------|-------------|-------------|-------------|-------------|
| `OdsStreamWriter` | 8,288 ms | 4,955 ms | 472.7 MB | 472.4 MB | 37.3 MB | 38.1 MB |
| `MiniExcel` | 7,990 ms | 4,920 ms | 3,352.5 MB | 3,358.3 MB | 47.0 MB | 47.6 MB |
| `ClosedXml` | 39,021 ms | 38,837 ms | 10,944.5 MB | 10,953.4 MB | 2,181.0 MB | 2,180.8 MB |

配置量與峰值工作集兩次幾乎一致；耗時在第 1 次偏高（冷行程／背景負載），故官方
對比表以第 2 次為準，並誠實列出兩次原始數字。
...
- **記憶體（峰值工作集）：OdsStreamWriter 仍明顯領先 DOM 路徑。** 第 2 次
  `38.1 MB` 對 `ClosedXml` 的 `2,180.8 MB`，約 57 倍；主因是串流寫入不把整份
  活頁簿常駐為物件圖。`MiniExcel` 同屬串流路線，峰值約 `47.6 MB`，與
  `OdsStreamWriter` 差距約 1.25 倍。公開敘事應強調「遠低於 DOM 方案的峰值
  工作集」，而非未加限定的「小於 1MB」口號（見 `AGENTS.md` 與本文件方針）。
- **耗時：熱路徑後已與 MiniExcel 接近持平。** 第 2 次 `OdsStreamWriter`
  （`4,955 ms`）與 `MiniExcel`（`4,920 ms`）僅差約 **0.7%**，在手動單次量測
  ...
  仍小（`8,288` 對 `7,990` ms），顯示相對關係穩定、絕對值易受冷啟動影響。
- **GC 累積配置量：`OdsStreamWriter` 明顯優於兩組對照。** 第 2 次約
  `472.4 MB`，約為 MiniExcel（`3,358.3 MB`）的七分之一、ClosedXML
  （`10,953.4 MB`）的二十三分之一；亦低於熱路徑前約 `770 MB` 的舊量測。累積
