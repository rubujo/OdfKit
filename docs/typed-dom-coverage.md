...
| `office:chart` 圖表根 facade | complete | `OfficeChartContentModelFacadeSupportsChartAppendAndEnumeration` |
| `office:image` 影像框架 facade | complete | `OfficeImageContentModelFacadeSupportsFrameAppendAndEnumeration` |
| `office:database` 元件容器 facade | complete | `OfficeDatabaseContentModelFacadeSupportsComponentContainers` |
...
- 已有所有 typed wrapper 共用的泛型 child facade，且 generated wrapper 已產生依 schema child element relation 展開的專屬 child collection property；Wave 1 已為 `office:text`、`table:table`、`draw:page`、四主格式 `office:presentation`／`office:drawing` 與次格式 `office:chart`／`office:image`／`office:database`／`office:spreadsheet` 新增手寫 content model facade；`oasis-odf14-dom-wrappers.json` manifest 供手動重產 DOM wrappers，未納入預設產生清單。
