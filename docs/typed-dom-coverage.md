...
- `TypedDomParityTests` 已用 ODFDOM 風格 user story 覆蓋 text body、spreadsheet table、image frame / SVG alternative text，以及巢狀清單 / 註腳的 typed 建立、schema-specific child collection traversal 與 XML round-trip。
...
- 已有 repo 內 ODFDOM 風格 sample usage parity tests，涵蓋 text、spreadsheet、draw image frame、巢狀清單與註腳 traversal；尚未對外部 ODF Toolkit / ODFDOM 官方 sample corpus 做逐項 API parity。
...
- 與 ODFDOM sample usage 對照的 user story tests，至少涵蓋文字、試算表、圖片 frame、巢狀清單與註腳 traversal；外部官方 sample corpus 仍需擴充。
