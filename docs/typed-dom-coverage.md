...
- `OdfElement` 已提供 schema-aware typed attribute helpers，涵蓋 `int`、`bool`、`decimal`、ODF 日期時間字串、`style:family`、`office:version` 與 MIME 類型。
- `DomWrappersCSharpWriter` 會從 RELAX NG `data` / `value` 節點推斷常用 datatype，讓 wrapper 可把 boolean、integer、decimal、date/dateTime、style family、ODF version 與 media type attribute 輸出為可空強型別屬性；未知或衝突型別會保守維持 `string?`。
...
- Attribute property 目前已有基底 typed helper，且 generated artifact 已包含 integer、decimal、date/dateTime、style family、ODF version 與 media type 強型別 property；boolean 類屬性多落在手寫 wrapper 或 schema 共用型別邊界，length 與其他 ODF datatype 仍需補齊。
