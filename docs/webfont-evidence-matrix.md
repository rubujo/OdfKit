...
| --- | --- | --- | --- | --- |
| 0 中性契約與 corpus | 已實證（工程） | 中性 sequence／IVS／PUA 契約、opaque font ID、實際 bytes SHA-256、版本化 JSON Profile、CNS EUC-TW provider、Windows EUDC `.tte`／`.ttf` 登錄來源 resolver、clean-room 紀錄；Windows resolver 的 `netstandard2.0` 資產已由 net8 與 net48 consumer 載入；WebFont 專案共用 `0.0.1` 與 pack；本機 managed-only nupkg 掃描通過 | CI 發布產物的 managed-only 掃描與授權漂移自動化；以合法 TrueType fixture 複製成 `.tte` 驗證副檔名路徑 | 個別字型法律審查、真實客戶 corpus、客戶合法 `EUDC.TTE` 實檔與登錄關聯驗收 |
