...
| 1 engine／format／browser | Experimental | 有界 sfnt／TTC／OTC parser、WOFF 與 null-transform WOFF2 輸入正規化、TrueType composite 與 GSUB output closure、`cmap` 4／12／14、TTF／OTF／zlib WOFF／net10 WOFF2、checksum、`fsType`；TrueType Variable Fonts 已實作 `gvar` short／long offset 重建與 `fvar`／`gvar` axis 驗證；standalone／OTC face 的 CID-keyed 靜態 CFF 1.0 已實作 INDEX／DICT／FDSelect／charset／Private／Subrs／Type 2 operand verifier；standalone／OTC face 且含 VariationStore 的 CFF2 variable 已實作 32-bit INDEX、DICT、FDSelect 0／3／4、ItemVariationData、`vsindex`、`blend`、subroutine 與 retain-GIDs；官方 Noto CJK OTC face、Source Han Sans CFF／CFF2、Noto Color Emoji bitmap／COLRv1 均產生 deterministic TTF／OTF／WOFF／WOFF2；真實 CNS 宋體／楷體 Ext-B／PUA、IPAmj IVS 與雙 CNS face TTC 成功；複雜文字與 color font 採保留完整 glyph ID space 的 correctness-first 路徑；Chromium／Firefox／WebKit 已完成 CFF、Arabic／Devanagari variable、Source Han CFF2 及 COLRv1 color 的來源／subset 像素差分，color 案例另要求非灰階像素 | transformed WOFF2 輸入；CFF／CFF2 尚缺 compact INDEX／DICT／subroutine 重寫、名稱式 CFF 與 coverage-guided fuzz；COLR／SVG／sbix 分格式 closure 與 aggressive pruning；CBDT bitmap-only 不可作 Firefox 跨瀏覽器輸出；擴大 variable／complex-script corpus | Safari 實機、第三方惡意字型安全稽核；名稱式 CFF、無 VariationStore 的 CFF2、SVG 主動內容與尚未具真實 corpus 的 color 模型 |
