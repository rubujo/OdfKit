...
| 1 engine／format／browser | Experimental | 有界 sfnt／TTC parser、TrueType composite 與 GSUB output closure、`cmap` 4／12／14、TTF／OTF／zlib WOFF／net10 WOFF2、checksum、`fsType`；TrueType Variable Fonts 已實作 `gvar` short／long offset 重建、未用 GID 清空及 `fvar`／`gvar` axis 驗證，真實 Arabic／Devanagari variable 已產生三格式；standalone CID-keyed 靜態 CFF 1.0 已實作 INDEX／DICT／FDSelect／charset／Private／Subrs 有界驗證與 retain-GIDs，Source Han Sans 2.005R 已產生 OTF／WOFF／WOFF2；真實 CNS 宋體／楷體 Ext-B／PUA、IPAmj IVS 與雙 CNS face TTC 成功；OTC、名稱式 CFF、CFF2 與 color 明確拒絕；複雜文字採 correctness-first 模式保留完整 glyph ID space、`cmap`、GDEF／GPOS／GSUB，來源與輸出的 layout tables 必須 byte-identical；真實 CFF 中文、Arabic／Devanagari 在 Chromium、Firefox 與 WebKit 的九組字串逐 RGBA byte 及文字 metrics 相同 | TrueType variable 尚缺三瀏覽器 variation axis 差分及更廣 mutation；CFF 尚缺 compact INDEX／DICT 重寫、名稱式 CFF 與 coverage-guided fuzz；擴大 complex-script corpus；尚未做 layout table 重寫或 aggressive glyph pruning | Safari 實機、第三方惡意字型安全稽核；OTC／名稱式 CFF／CFF2／PostScript variable／color 維持不支援 |
