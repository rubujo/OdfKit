...
- **試算表／簡報／繪圖範本 Round-trip 測試**：於 `TemplateRoundTripTests` 新增
  `SpreadsheetTemplateInstantiationAndRoundTrip`／`PresentationTemplateInstantiationAndRoundTrip`／
  `DrawingTemplateInstantiationAndRoundTrip`，比照既有文字範本測試涵蓋母片頁面、
  `OdfTemplateMetadata`、內容的儲存／載入 Round-trip。
- **主控文件合併／條件式載入整合測試**：於 `MasterDocumentTests` 新增
  `MergeSubDocuments_IncludesOnRequestSubDocuments`（驗證 `xlink:actuate="onRequest"` 的子文件
  合併時仍會完整展開，不受即時檢視語意影響）與
  `MergeSubDocuments_RenamesConflictingStyleNames`（驗證主控文件與子文件同名衝突樣式會依
  `OdfMergeOptions.StyleConflictResolution` 重新命名）。
  **此測試過程中發現並修正兩個真實缺陷**：
  1. `TextMasterDocument.MergeSubDocuments` 原先只複製子文件的樣式（透過 `AppendDocument`），
     從未複製主控文件「自身」定義的樣式 —— 主控文件若有自訂樣式，合併後會整份遺失。
     已修正：新增 `OdfDocumentMergeEngine.MergeStyles`（由 `private` 改為 `internal`，
     供 `MergeSubDocuments` 在處理本文內容前先行呼叫，僅合併樣式不含內容節點），
     並對主控文件自身內容套用 `OdfDocumentStyleRemapEngine.RemapStylesInNodes` 重新導向。
  2. `OdfDocumentMergeEngine.MergeStyles` 透過原始 DOM 操作（`AppendChild`）寫入重新命名後的
     衝突樣式節點，未呼叫 `OdfStyleEngine.RebuildStyleIndex()` 重建索引快取 —— 導致對同一目標
     文件**連續呼叫兩次以上**合併（例如本批新增的「先合併主控文件自身樣式，再依序合併各子文件」）時，
     後續合併呼叫的衝突偵測會讀到過期快取而誤判為「不衝突」，導致重複樣式名稱未被重新命名
     （schema 上仍合法但語意錯誤，因為兩個不同樣式定義會共用同一個名稱，後者會覆蓋前者的外觀）。
     已修正：於 `MergeStyles` 結尾呼叫 `dest.StyleEngine.RebuildStyleIndex()`。
     此修正影響所有透過 `OdfDocument.AppendDocument` 對同一目標文件進行多次合併呼叫的既有使用情境，
     不僅限於主控文件合併。
- **Flat XML 大檔案效能基準測試**：已於 Batch 6（X-2）新增
  `FlatXmlMediaEncodingTests.TempStreamFactory_SwitchesBetweenMemoryAndTempFileByThreshold`，
  驗證大型文件門檻判斷邏輯；既有 `LargeMediaFlatXmlStreamingRoundTrip`（5 MB 圖片）已涵蓋
  二進位內容串流路徑的功能正確性。完整的「大檔案轉換耗時」效能基準測試（量測絕對效能數據而非
  正確性）非本專案既有測試慣例範圍（既有測試套件不含效能基準測試類別），故不在此新增。
