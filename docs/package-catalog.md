...
| `OdfKit.WebFonts.OpenType` | 字型引擎 | `net10.0;netstandard2.0` | 純 .NET TTF／OTF／TTC／OTC／TTE／WOFF 輸入、net10 standalone WOFF2 null／`glyf`／`loca`／`hmtx` transform、net10 WOFF2 collection 指定 face、standalone／OTC face 的 CID-keyed 與名稱式靜態 CFF 1.0、名稱式 CFF `seac` closure、含 VariationStore 的 CFF2 variable、不含 VariationStore 的非變動 CFF2、color correctness-first 子集化；輸出 TTF／OTF／WOFF、net10 WOFF2 | Variable／CFF／CFF2／color 與 WOFF2 collection 輸入能力為 experimental；巢狀或找不到 StandardEncoding 元件的 `seac`、缺少 VariationStore 卻使用 `vsindex`／`blend` 的 CFF2 與直接 collection 輸出明確拒絕 |
