...
- `CliTests.SanitizeRemovesMacroArtifactsToOutputFile` 驗證 CLI `sanitize` 會移除
  Basic / Scripts / macro signature / document signature artifact 與 macro URI，同時保留一般
  HTTPS 外部資源連結與安全封裝內資源。
