...
|---|---|---|---|---|
| `ODS-PACKAGE-001` | ODS | PackageFidelity | round-trip-verified | Il round trip del pacchetto non implica ricalcolo delle formule o semantica completa. |
| `ODS-SEMANTIC-001` | ODS | SemanticApiDepth | semantic-contract-verified | Legge valori e formule salvati, ma non ricalcola. |
| `ODT-SEMANTIC-001` | ODT | SemanticApiDepth | semantic-contract-verified | Nessun motore di layout o rendering. |
| `ODP-SEMANTIC-001` | ODP | SemanticApiDepth | semantic-contract-verified | Carico DOM/pacchetto; nessuna API streaming per diapositive. |
| `ODG-SEMANTIC-001` | ODG | SemanticApiDepth | semantic-contract-verified | Nessun layout SmartArt o rendering a livello di pixel. |
| `ODF-INTEROP-001` | ODF | InteropEvidence | interop-tested | Una versione LibreOffice provata non garantisce uguaglianza pixel in ogni suite. |
...
Lo schema v4 della copertura semantica richiede inoltre, per ogni argomento, prove relative a
