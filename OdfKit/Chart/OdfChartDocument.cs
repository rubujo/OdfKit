...
public partial class OdfChartDocument(OdfPackage package, string subPath) : OdfDocument(package, subPath)
{
    ...
    public new static Task<OdfChartDocument> LoadAsync(string path) => LoadAsync(path, default);

    /// <summary>
    /// Additional public overload without optional parameters.
    /// 不含選用參數的公開多載。
    /// </summary>
    public new static async Task<OdfChartDocument> LoadAsync(string path, CancellationToken cancellationToken) =>
    ...
    /// <returns>The loaded <see cref="OdfChartDocument"/> instance. / 載入完成的 <see cref="OdfChartDocument"/> 執行個體。</returns>
    /// <exception cref="InvalidOperationException">When the specified document is not an ODC chart. / 當指定文件不是 ODC 圖表時擲出。</exception>
    public new static OdfChartDocument Load(Stream stream) => Load(stream, null);

    /// <summary>
    /// Additional public overload without optional parameters.
    /// 不含選用參數的公開多載。
    /// </summary>
    public new static OdfChartDocument Load(Stream stream, string? fileName)
    {
    ...
    /// </summary>
    /// <returns>A task representing the asynchronous load operation, whose result is the loaded <see cref="OdfChartDocument"/>. / 代表非同步載入作業的工作，其結果為載入完成的 <see cref="OdfChartDocument"/>。</returns>
    public new static Task<OdfChartDocument> LoadAsync(Stream stream) => LoadAsync(stream, null, default);
    ...
    public new static Task<OdfChartDocument> LoadAsync(Stream stream, CancellationToken cancellationToken) => LoadAsync(stream, null, cancellationToken);

    /// <summary>
    /// Additional public overload without optional parameters.
    /// 不含選用參數的公開多載。
    /// </summary>
    public new static Task<OdfChartDocument> LoadAsync(Stream stream, string? fileName) => LoadAsync(stream, fileName, default);

    /// <summary>
    /// Additional public overload without optional parameters.
    /// 不含選用參數的公開多載。
    /// </summary>
    public new static async Task<OdfChartDocument> LoadAsync(Stream stream, string? fileName, CancellationToken cancellationToken) =>
