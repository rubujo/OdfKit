...
public partial class OdfChartDocument
{
    ...
    /// 取得圖表內嵌資料表的延遲載入快照。
    /// </summary>
    /// <returns>The local data table snapshot; an empty snapshot if the chart has no embedded data table. / 內嵌資料表快照；若圖表沒有內嵌資料表，則回傳空快照。</returns>
