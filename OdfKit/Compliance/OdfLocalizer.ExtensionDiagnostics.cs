...
public static partial class OdfLocalizer
{
    private static readonly Dictionary<string, Dictionary<string, string>> SupplementalDiagnosticsDictionaries = new(StringComparer.OrdinalIgnoreCase)
    {
        ["en"] = new(StringComparer.Ordinal)
        {
            ...
            ["Diag_XlsxToOdfConverter_SharedStringIndexOutOfRange"] = "XLSX shared string index {0} is out of range (table size {1}); the cell value was cleared.",
            ...
            ["Msg_Practical_MacroOrScript"] = "The document contains macros or scripts that are not portable across office suites.",
            ["Msg_Practical_NonPortableImage"] = "The document contains an image media type that may not remain editable or visible across office suites.",
            ["Msg_Practical_NestedTextBox"] = "The document contains nested text boxes, which commonly change layout during cross-suite editing.",
            ["Msg_Practical_ComplexGraphicGroup"] = "The document contains nested graphic groups, which may be flattened or regrouped by another office suite.",
            ["Msg_Practical_StyleFragmentation"] = "The document contains a large number of automatic styles, which may increase editing and compatibility risk.",
            ["Msg_Practical_AdvancedChart"] = "The document contains advanced chart features such as bubble, stock, 3D, wall or floor settings that may render differently outside LibreOffice.",
            ["Msg_Practical_HeaderFooterLayout"] = "The document contains header or footer layout settings that may change when edited in Microsoft Office.",
            ["Msg_Practical_WordOdtRepairRisk"] = "The text document combines structures that may trigger a Microsoft Word ODT repair prompt.",
            ["Msg_Practical_SpreadsheetSizing"] = "The spreadsheet contains explicit column widths or row heights that may be adjusted by another office suite.",
            ["Msg_Practical_SpreadsheetPrintSettings"] = "The spreadsheet contains print settings that should be verified after cross-suite editing.",
            ["Msg_Practical_ImageTransform"] = "The document contains cropped or rotated images that may render differently during portable editing.",
            ["Msg_ImageInspection_NonPortableMediaType"] = "The image media type is not ideal for portable editing.",
            ["Msg_ImageInspection_MissingAltText"] = "The image frame does not contain alternative text.",
            ["Msg_ImageInspection_LargeImage"] = "The image is large and may make editing slower.",
            ["Msg_ImageInspection_Transform"] = "The image uses crop or rotation settings.",
            ["Msg_ImageInspection_DuplicateBytes"] = "The image bytes duplicate another package image.",
        ...
        ["zh-TW"] = new(StringComparer.Ordinal)
        {
            ...
            ["Msg_Practical_MacroOrScript"] = "文件包含巨集或腳本，跨辦公套件編輯時不具可攜性。",
            ["Msg_Practical_NonPortableImage"] = "文件包含可能無法在跨辦公套件情境保持可編輯或可見的影像媒體類型。",
            ["Msg_Practical_NestedTextBox"] = "文件包含巢狀文字方塊，跨套件編輯時常見版面變動。",
            ["Msg_Practical_ComplexGraphicGroup"] = "文件包含巢狀圖形群組，其他辦公套件可能會將其扁平化或重新分組。",
            ["Msg_Practical_StyleFragmentation"] = "文件包含大量自動樣式，可能增加編輯與相容性風險。",
            ["Msg_Practical_AdvancedChart"] = "文件包含泡泡圖、股票圖、3D、牆面或地板等進階圖表功能，在 LibreOffice 以外可能有呈現差異。",
            ["Msg_Practical_HeaderFooterLayout"] = "文件包含頁首或頁尾版面設定，在 Microsoft Office 編輯後可能產生變動。",
            ["Msg_Practical_WordOdtRepairRisk"] = "文字文件組合了可能觸發 Microsoft Word ODT 復原提示的結構。",
            ["Msg_Practical_SpreadsheetSizing"] = "試算表包含明確欄寬或列高設定，其他辦公套件可能會重新調整。",
            ["Msg_Practical_SpreadsheetPrintSettings"] = "試算表包含列印設定，跨套件編輯後應再次確認。",
            ["Msg_Practical_ImageTransform"] = "文件包含裁切或旋轉圖片，在可攜編輯流程中可能有呈現差異。",
            ["Msg_ImageInspection_NonPortableMediaType"] = "圖片媒體類型不適合可攜編輯流程。",
            ["Msg_ImageInspection_MissingAltText"] = "圖片框架缺少替代文字。",
            ["Msg_ImageInspection_LargeImage"] = "圖片檔案較大，可能降低編輯速度。",
            ["Msg_ImageInspection_Transform"] = "圖片使用裁切或旋轉設定。",
            ["Msg_ImageInspection_DuplicateBytes"] = "圖片位元組內容與封裝內其他圖片重複。",
        ...
        ["de"] = new(StringComparer.Ordinal)
        {
            ...
            ["Diag_XlsxToOdfConverter_SharedStringIndexOutOfRange"] = "Der XLSX-Shared-String-Index {0} liegt außerhalb des Bereichs (Tabellengröße {1}); der Zellenwert wurde gelöscht.",
        ...
        ["fr"] = new(StringComparer.Ordinal)
        {
            ...
            ["Diag_XlsxToOdfConverter_SharedStringIndexOutOfRange"] = "L'index de chaîne partagée XLSX {0} est hors limites (taille du tableau {1}) ; la valeur de la cellule a été effacée.",
        ...
        ["nl"] = new(StringComparer.Ordinal)
        {
            ...
            ["Diag_XlsxToOdfConverter_SharedStringIndexOutOfRange"] = "XLSX shared-string-index {0} valt buiten het bereik (tabelgrootte {1}); de celwaarde is gewist.",
        ...
        ["nb"] = new(StringComparer.Ordinal)
        {
            ...
            ["Diag_XlsxToOdfConverter_SharedStringIndexOutOfRange"] = "XLSX delt streng-indeks {0} er utenfor området (tabellstørrelse {1}); celleverdien ble tømt.",
        ...
        ["pt"] = new(StringComparer.Ordinal)
        {
            ...
            ["Diag_XlsxToOdfConverter_SharedStringIndexOutOfRange"] = "O índice de cadeia compartilhada XLSX {0} está fora do intervalo (tamanho da tabela {1}); o valor da célula foi limpo.",
        ...
        ["it"] = new(StringComparer.Ordinal)
        {
            ...
            ["Diag_XlsxToOdfConverter_SharedStringIndexOutOfRange"] = "L'indice della stringa condivisa XLSX {0} è fuori intervallo (dimensione tabella {1}); il valore della cella è stato cancellato.",
        ...
        ["sk"] = new(StringComparer.Ordinal)
        {
            ...
            ["Diag_XlsxToOdfConverter_SharedStringIndexOutOfRange"] = "Index zdieľaného reťazca XLSX {0} je mimo rozsahu (veľkosť tabuľky {1}); hodnota bunky bola vymazaná.",
        ...
        ["da"] = new(StringComparer.Ordinal)
        {
            ...
            ["Diag_XlsxToOdfConverter_SharedStringIndexOutOfRange"] = "XLSX delt streng-indeks {0} er uden for interval (tabelstørrelse {1}); celleværdien blev ryddet.",
        ...
        ["ms"] = new(StringComparer.Ordinal)
        {
            ...
            ["Diag_XlsxToOdfConverter_SharedStringIndexOutOfRange"] = "Indeks rentetan kongsi XLSX {0} berada di luar julat (saiz jadual {1}); nilai sel telah dikosongkan.",
        ...
        ["ko"] = new(StringComparer.Ordinal)
        {
            ...
            ["Diag_XlsxToOdfConverter_SharedStringIndexOutOfRange"] = "XLSX 공유 문자열 인덱스 {0}이(가) 범위를 벗어났습니다(테이블 크기 {1}). 셀 값이 지워졌습니다.",
