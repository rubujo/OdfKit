...
{
    /// <summary>
    /// Performs package-level ODF validation before full schema validation is available.
    /// </summary>
    public static class OdfPackageValidator
    {
        private static readonly Dictionary<string, string> ExpectedRootNames = new Dictionary<string, string>(StringComparer.Ordinal)
        {
        ...
        private static readonly Dictionary<string, string> ExpectedManifestMediaTypes = new Dictionary<string, string>(StringComparer.Ordinal)
        {
        ...
        /// <summary>
        /// Validates an ODF package against package-level rules and an optional profile.
        /// </summary>
            ...
            List<OdfValidationIssue> issues)
        {
            ...
            if (package.Entries.TryGetValue("mimetype", out OdfPackageEntry? mimeEntry) &&
                mimeEntry.WasStoredInZip == false)
            {
                issues.Add(new OdfValidationIssue(
                    OdfIssueSeverity.Error,
                    "ODF0004",
        ...
        {
            foreach (string entryName in new[] { "content.xml", "styles.xml", "meta.xml", "settings.xml" })
            {
        ...
        private static IEnumerable<KeyValuePair<string, string>> GetExpectedXmlRootEntries(OdfPackage package)
        {
            ...
            foreach (string entryName in package.Entries.Keys.OrderBy(name => name, StringComparer.Ordinal))
            {
                ...
                if (expectedRoot != null && seen.Add(normalized))
                {
        ...
        private sealed class XmlRootInfo
        {
            public XmlRootInfo(string namespaceUri, string localName, string? version)
            {
