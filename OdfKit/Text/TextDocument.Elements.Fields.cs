...
public partial class TextDocument
{
    ...
    /// 新增一個標題至文件本文結尾。
    /// </summary>
    /// <param name="text">The heading's text content. / 標題的文字內容。</param>
    /// <param name="outlineLevel">The heading's outline level. / 標題的大綱階層。</param>
    ...
    /// </summary>
    /// <param name="styleName">The list style name, which must be unique. / 清單樣式名稱，必須唯一。</param>
    /// <param name="levels">The style settings for each level; the Level property must start at 1 and increase consecutively. / 各層級的樣式設定；Level 屬性需從 1 開始連續遞增。</param>
    ...
    /// <returns>A dictionary of field values indexed by field name. / 依欄位名稱索引的欄位值字典。</returns>
    /// <remarks>
