...
public class OdfIndexTemplateBuilder(OdfNode template)
{
    ...
    /// <summary>
    /// Short overload of AddTabStop that uses default values for all optional parameters and forwards to the full overload.
    /// 便利多載：AddTabStop 的所有可選參數使用預設值並轉呼叫最長多載。
    /// </summary>
    public OdfIndexTemplateBuilder AddTabStop() => AddTabStop("right", '.');

    /// <summary>
    /// Short overload of AddTabStop that accepts type; remaining optional parameters use defaults and forward to the full overload.
    /// 便利多載：提供 type；其餘可選參數使用預設值並轉呼叫最長 AddTabStop 多載。
    /// </summary>
