...
- WebFont 引擎、verifier 與耐久快取改共用 `WebFontUnicodePolicy`，修正空白、variation selector 與 default-ignorable 的 glyph 判定漂移；此問題先後讓真實 CFF 矩陣在 `VerifyContainsSequences` 與 `VerifyRetainsGlyphIds` 誤判合法產物，並可能讓 IVS／ZWJ 耐久快取回 500。
