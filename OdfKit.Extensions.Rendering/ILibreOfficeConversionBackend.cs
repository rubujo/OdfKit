...
public interface ILibreOfficeConversionBackend
{
    ...
    /// </summary>
    /// <param name="input">The source or target object. / 包含來源文件內容的輸入資料流</param>
    /// <param name="inputExtension">The text or value. / 來源文件的副檔名（例如 <c>odt</c>、<c>ods</c> 等）</param>
    /// <param name="convertTo">The value to use. / 要轉換的目標格式副檔名（例如 <c>pdf</c>、<c>png</c> 等）</param>
    /// <param name="ct">The value to use. / 用於取消作業的取消語彙</param>
    /// <returns>The result. / 包含轉換後文件內容的結果資料流</returns>
