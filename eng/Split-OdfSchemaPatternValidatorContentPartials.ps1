...
$sourcePath = Join-Path $complianceDir 'OdfSchemaPatternValidator.Content.cs'
$lines = [System.Collections.Generic.List[string]]@(Get-Content -Path $sourcePath -Encoding UTF8)
$lineCount = $lines.Count

$files = @(
    @{ Start = 12; End = 194; File = 'OdfSchemaPatternValidator.Content.Sequence.cs'; Region = 'Content Matching - Sequence' }
    @{ Start = 196; End = 393; File = 'OdfSchemaPatternValidator.Content.Interleave.cs'; Region = 'Content Matching - Interleave' }
    @{ Start = 395; End = ($lineCount - 3); File = 'OdfSchemaPatternValidator.Content.Repetition.cs'; Region = 'Content Matching - Repetition' }
)
...
function Get-UsingsForBlock {
    ...
    if ($Text -match '\.Linq|\.Any\(|\.All\(') { [void]$needed.Add('System.Linq') }
    if ($Text -match 'XElement|XAttribute') { [void]$needed.Add('System.Xml.Linq') }

    $order = @('System', 'System.Collections.Generic', 'System.Globalization', 'System.Linq', 'System.Xml.Linq')
    $result = @()
    foreach ($u in $order) {
...
Write-Host 'Removed OdfSchemaPatternValidator.Content.cs'
Write-Host 'Done.'