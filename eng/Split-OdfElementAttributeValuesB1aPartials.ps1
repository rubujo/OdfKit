...
$sourcePath = Join-Path $domDir 'OdfElement.AttributeValues.B1a.cs'
$lines = [System.Collections.Generic.List[string]]@(Get-Content -Path $sourcePath -Encoding UTF8)
$lineCount = $lines.Count

$files = @(
    @{ Start = 11; End = 269; File = 'OdfElement.AttributeValues.B1a1.cs'; Region = 'Attribute Values (B1a1)' }
    @{ Start = 271; End = ($lineCount - 3); File = 'OdfElement.AttributeValues.B1a2.cs'; Region = 'Attribute Values (B1a2)' }
)
...
Write-Host 'Removed OdfElement.AttributeValues.B1a.cs'
Write-Host 'Done.'