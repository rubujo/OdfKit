...
$sourcePath = Join-Path $formulaDir 'DefaultFormulaEvaluator.DatabaseFinancial.cs'
$lines = [System.Collections.Generic.List[string]]@(Get-Content -Path $sourcePath -Encoding UTF8)
$lineCount = $lines.Count

$files = @(
    @{ Start = 13; End = 195; File = 'DefaultFormulaEvaluator.Database.cs'; Region = 'Database Functions' }
    @{ Start = 197; End = ($lineCount - 3); File = 'DefaultFormulaEvaluator.Financial.cs'; Region = 'Financial Functions' }
)
...
Write-Host 'Removed DefaultFormulaEvaluator.DatabaseFinancial.cs'
Write-Host 'Done.'