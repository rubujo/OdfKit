...
$sourcePath = Join-Path $formulaDir 'DefaultFormulaEvaluator.Math.cs'
$lines = [System.Collections.Generic.List[string]]@(Get-Content -Path $sourcePath -Encoding UTF8)
$lineCount = $lines.Count

$files = @(
    @{ Start = 13; End = 213; File = 'DefaultFormulaEvaluator.Math.Rounding.cs'; Region = 'Math Functions - Rounding & Random' }
    @{ Start = 215; End = 344; File = 'DefaultFormulaEvaluator.Math.Trigonometry.cs'; Region = 'Math Functions - Trigonometry & Products' }
    @{ Start = 346; End = ($lineCount - 3); File = 'DefaultFormulaEvaluator.Math.Standard.cs'; Region = 'Math Functions - Standard' }
)
...
function Get-UsingsForBlock {
    ...
    if ($Text -match 'OdfNode|OdfPackage') { [void]$needed.Add('OdfKit.Core') }
...
# Keep a minimal stub or remove - delete original and only use split files
...
Write-Host 'Removed DefaultFormulaEvaluator.Math.cs'
Write-Host 'Done.'