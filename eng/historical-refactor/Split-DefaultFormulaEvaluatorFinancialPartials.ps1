...
$sourcePath = Join-Path $formulaDir 'DefaultFormulaEvaluator.Financial.cs'
$lines = [System.Collections.Generic.List[string]]@(Get-Content -Path $sourcePath -Encoding UTF8)
$lineCount = $lines.Count

$files = @(
    @{ Start = 12; End = 332; File = 'DefaultFormulaEvaluator.Financial.Annuity.cs'; Region = 'Financial Functions - Annuity' }
    @{ Start = 334; End = ($lineCount - 3); File = 'DefaultFormulaEvaluator.Financial.Interest.cs'; Region = 'Financial Functions - Interest' }
)
...
function Get-UsingsForBlock {
    ...
    if ($Text -match 'AstNode|LiteralNode|OdfFormulaError') { [void]$needed.Add('OdfKit.Formula.AST') }

    $order = @('System', 'System.Collections.Generic', 'OdfKit.DOM', 'OdfKit.Formula.AST')
    $result = @()
    foreach ($u in $order) {
...
Write-Host 'Removed DefaultFormulaEvaluator.Financial.cs'
Write-Host 'Done.'