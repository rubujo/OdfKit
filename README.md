...
- ODC 已有圖表類型、標題、圖例、座標軸標題、資料來源標籤、分類與序列 API；ODF formula 已有 MathML XML 讀寫、基礎 token helper、根節點與純文字摘要 API；ODI 已有主要影像寫入、摘要、讀回與框架版面 API；ODB 已有連線、資料表與查詢查找 / 移除 API。
...
- CLI 提供 `validate`、`validate-corpus`、`info`、`metadata`、`sanitize`、`convert-flat` 與 `pack`，其中驗證命令可輸出文字或 JSON，支援 CI 失敗條件，並可選用外部 ODF Validator baseline 與 documented exceptions。

完整格式矩陣請見 [docs/odf-format-support.md](docs/odf-format-support.md)，ODF 1.4 覆蓋狀態請見 [docs/odf14-coverage.md](docs/odf14-coverage.md)，ODF Toolkit 對標線請見 [docs/odf-toolkit-parity.md](docs/odf-toolkit-parity.md)，官方 corpus 來源請見 [docs/odf-official-corpus-sources.md](docs/odf-official-corpus-sources.md)，typed DOM 覆蓋狀態請見 [docs/typed-dom-coverage.md](docs/typed-dom-coverage.md)。
