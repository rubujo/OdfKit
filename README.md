...
- CLI 提供 `validate`、`info`、`metadata`、`convert-flat` 與 `pack`，其中 `validate` 可輸出文字或 JSON，支援 CI 失敗條件，並可選用外部 ODF Validator baseline。

完整格式矩陣請見 [docs/odf-format-support.md](docs/odf-format-support.md)，ODF 1.4 覆蓋狀態請見 [docs/odf14-coverage.md](docs/odf14-coverage.md)，ODF Toolkit 對標線請見 [docs/odf-toolkit-parity.md](docs/odf-toolkit-parity.md)。
