...
public sealed class Type2CharStringVerifierTests
{
    ...
    [Fact]
    public void Verify_RejectsSeacReachedThroughLocalSubroutine()
    {
        byte[] charString = [32, 10, 14];
        ReadOnlyMemory<byte>[] localSubroutines = [new byte[] { 139, 139, 139, 139, 14 }];

        NotSupportedException exception = Assert.Throws<NotSupportedException>(() =>
            Type2CharStringVerifier.Verify(charString, [], localSubroutines));

        Assert.Contains("cff-seac", exception.Message, StringComparison.Ordinal);
    }
